            ...
            <React.Fragment>
                <Card style={{ border: 'none' }} className="mt-3">
                    <Card.Body className="mt-2">
                        <Link style={{ textDecoration: 'none', color: 'black' }} to={"/lista/projetos"}>
    ...
    loadFillProjetos() {
        return this.state.listaProjetos.slice(0, 6).map((data, index) => {
            return data.users.map((data1, index1) => {
                if (data1.id === this.state.id && data.projeto.estado === false) {
                    if (data.projeto.estado === false) {
                        return (
                            <li key={index, index1} style={{ marginTop: -10 }} className="list-group-item border-0">
